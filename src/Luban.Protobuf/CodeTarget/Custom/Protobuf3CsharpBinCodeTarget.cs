...
[CodeTarget("protobuf3-custom-cs-bin")]
public class Protobuf3CsharpBinCodeTarget : CsharpCodeTargetBase
{
  ...
  {
    var template = GetTemplate("tables-bin");
  ...
  {
    var template = GetTemplate("table-bin");
    var tplCtx = CreateTemplateContext(template);
    // 添加自定义模板
    tplCtx.PushGlobal(new ProtobufCustomTemplateExtension());
    var extraEnvs = new ScriptObject
